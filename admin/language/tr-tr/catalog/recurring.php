<?php

// Heading
$_['heading_title']         = 'Abonelikler';

// Text
$_['text_success']          = 'Başarılı: Abonelikler başarılı bir şekilde değiştirildi!';
$_['text_list']             = 'Abonelik Listesi';
$_['text_add']              = 'Abonelik Ekle';
$_['text_edit']             = 'Abonelik Düzenle';
$_['text_day']              = 'Gün';
$_['text_week']             = 'Hafta';
$_['text_semi_month']       = 'Yarım Ay';
$_['text_month']            = 'Ay';
$_['text_year']             = 'Yıl';
$_['text_recurring']        = '<p><i class="fa fa-info-circle"></i> Abonelik tutarları, döngüleri ve sıklığı hesaplanır.</p><p>Örneğin kullanıcı 2 haftada faturalandırılırsınız sonra bir frekans "haftanın" ve "2", bir döngü kullanırsanız.</p><p>Süresi kaç kez kullanıcı, bir ödeme yapacak olan bu set iptal edilir kadar ödemeleri isterseniz 0.</p>';
$_['text_profile']          = 'Abonelik Profili';
$_['text_trial']            = 'Deneme profili';

// Entry
$_['entry_name']            = 'Adı';
$_['entry_price']           = 'Fiyat';
$_['entry_duration']        = 'Süre';
$_['entry_cycle']           = 'Döngüsü';
$_['entry_frequency']       = 'Sıklık';
$_['entry_trial_price']     = 'Deneme fiyatı';
$_['entry_trial_duration']  = 'Deneme süresi';
$_['entry_trial_status']    = 'Deneme durumu';
$_['entry_trial_cycle']     = 'Deneme döngüsü';
$_['entry_trial_frequency'] = 'Deneme sıklığı';
$_['entry_status']          = 'Durumu';
$_['entry_sort_order']      = 'Sıralama';

// Column
$_['column_name']           = 'Profil Adı';
$_['column_sort_order']     = 'Sıralama';
$_['column_action']         = 'Eylem';

// Error
$_['error_warning']         = 'Uyarı: Oluşan hatalar için lütfen formu dikkatli kontrol ediniz!';
$_['error_permission']      = 'Uyarı: Abonelikleri düzenleme iznine sahip değilsiniz!';
$_['error_name']            = 'Profil Adı 3 ile 255 karakter arasında olmalı!';
$_['error_product']         = 'Uyarı: Bu profili silemezsiniz, %s ürün tarafından kullanılıyor!';